  ...
  observer = new MutationObserver((mutations) => {
    ...
    for (const m of mutations) {
      for (const node of [...m.addedNodes, ...m.removedNodes]) {
        ...
        if (SENTINEL_SELECTORS.includes(tag) || SENTINEL_SELECTORS.some(s => node.classList?.contains(s))) {
          changed = true;
          break;
        }
      }
...
export function buildSettingsTab(container) {
  ...
  monRow.style.cssText = 'display:flex;align-items:center;justify-content:space-between;margin-top:10px;';
  ...
  note.style.cssText = 'margin-top:10px;padding:8px 10px;background:rgba(200,170,110,0.04);border:1px solid #1a2535;font-size:10px;color:#3a5060;line-height:1.6;';
  const noteSpan = document.createElement('span');
  noteSpan.style.color = '#785a28';
  noteSpan.textContent = 'Note:';
  note.appendChild(noteSpan);
  note.appendChild(document.createTextNode(' Values are also refreshed when you switch to the Visual Builder tab. Elements not present in the current screen show a dim "not in DOM" indicator.'));
  ...
  dangerDesc.style.cssText = 'font-size:10px;color:#3a5060;line-height:1.5;margin-top:6px;margin-bottom:12px;';
  dangerDesc.textContent = 'Permanently deletes all saved CSS and settings from DataStore. The plugin will reload clean.';
  ...
  deleteBtn.style.cssText = 'padding:7px 14px;font-family:inherit;font-size:10px;letter-spacing:0.08em;text-transform:uppercase;background:transparent;border:1px solid #5a2828;color:#c84b4b;cursor:pointer;transition:background 0.15s;';
  deleteBtn.textContent = 'Delete All Saved Data';
  deleteBtn.addEventListener('mouseenter', () => { deleteBtn.style.background = 'rgba(200,75,75,0.1)'; });
  ...
  deleteStatus.style.cssText = 'font-size:10px;color:#4caf82;opacity:0;transition:opacity 0.2s;';
  ...
  deleteBtn.addEventListener('click', async () => {
    ...
    if (ok) {
      ...
      setTimeout(() => { deleteStatus.style.opacity = '0'; }, 2500);
    }
  });
  ...
  aboutContent.style.cssText = 'font-size:10px;color:#3a5060;line-height:1.8;margin-top:8px;';

  const aboutLines = [
    ['Plugin:',        'Snooze-CSS by Reformed Doge'],
    ['Catalog:',       '~470 elements · 44 groups'],
    ['Generic Tools:', 'Omni Inspector · Hover-Reveal · Clean Mode · Client Frame · Hue-Rotate · Font · CSS Vars · Gradient BG · Screen Tint · Root Overlay · Glass Panel · Mask Fade · Local Asset · BG Replace · Img Replace · Hide · Color · Scrollbar · Custom CSS'],
    ['Screens:',       'Home, Lobby, Champ Select, Post-Game, Store, Loot, Collections, TFT, Clash, Events +more'],
  ];
