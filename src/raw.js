...
const CSS_PROPS = Array.from(window.getComputedStyle(document.documentElement))
  .filter(prop => !prop.startsWith('-webkit-') || prop === '-webkit-mask' || prop === '-webkit-app-region')
  .sort();

// FALLBACK VALUES
const FALLBACK_VALUES = {
  'display':['none','block','flex','grid','inline','inline-block','inline-flex','inline-grid','contents'],
  ...
  'width':['auto','100%','100vw','max-content','min-content','fit-content','0','100px'],
  'height':['auto','100%','100vh','max-content','min-content','fit-content','0','100px'],
  ...
  'grid-template-columns':['1fr','repeat(auto-fill, minmax(100px, 1fr))','auto','100%'],
  ...
  'color':['transparent','currentColor','inherit','initial','#c8aa6e','#f0e6d3','#a0b4c8','#000','#fff'],
  'background-color':['transparent','currentColor','inherit','initial','rgba(0,0,0,0.5)','#060e1a','#0a1428','#c8aa6e','#000','#fff'],
  'background-size':['cover','contain','auto','100%','100% 100%'],
  'background-repeat':['no-repeat','repeat','repeat-x','repeat-y','space','round'],
  'background-position':['center','top','bottom','left','right','center center','center top','center bottom'],
  ...
  'filter':['none','blur(4px)','blur(10px)','grayscale(1)','grayscale(0.5)','brightness(0.5)','brightness(1.5)','hue-rotate(90deg)','hue-rotate(180deg)','saturate(0)','saturate(2)','sepia(1)','invert(1)','contrast(1.5)','drop-shadow(0 0 5px #000)'],
  'backdrop-filter':['none','blur(4px)','blur(8px)','blur(16px)','blur(24px)','blur(8px) brightness(0.7)','blur(120px) brightness(0.2)'],
  'box-shadow':['none','0 0 10px rgba(0,0,0,0.5)','0 4px 8px rgba(0,0,0,0.3)','inset 0 0 10px #000'],
  'text-shadow':['none','0 0 10px #c8aa6e','0 0 5px #000','1px 1px 2px #000'],
  'transform':['none','scale(1)','scale(0.8)','scale(1.2)','rotate(0deg)','rotate(90deg)','translateX(0)','translateY(0)','translate(-50%, -50%)'],
  'transform-origin':['center','top','bottom','left','right','top left','top right','bottom left','bottom right'],
  'transition':['none','all 0.2s','all 0.3s ease','opacity 0.2s','0.2s','0.3s','0.5s'],
  'animation':           ['none'],
  'animation-direction': ['normal','reverse','alternate','alternate-reverse'],
  'animation-fill-mode':['none','forwards','backwards','both'],
  'animation-play-state':['running','paused'],
  'animation-timing-function':['ease','linear','ease-in','ease-out','ease-in-out','step-start','step-end'],
  'cursor':['auto','default','pointer','text','move','not-allowed','grab','crosshair','none'],
...
async function initMDN() {
  try {
    ...
    console.log('[Snooze-CSS] MDN Autocomplete Data Loaded Successfully! (Using dynamic values + fallback)');
  } catch (err) {
    console.warn('[Snooze-CSS] MDN data failed to load. Falling back to offline dictionary.', err);
  }
}
initMDN();
...
export function buildRawTab(container) {
  ...
  textareaEl.addEventListener('scroll',  () => { hideAC(); updateScrollBtns(); });
...
function setTabSize(n) {
  ...
  const el = document.getElementById('ci-tabsize-val');
  if (el) el.textContent = tabSize;
...
function updateScrollBtns() {
  ...
  const btnTop    = document.getElementById('ci-scroll-top');
  const btnBottom = document.getElementById('ci-scroll-bottom');
  ...
  btnBottom.style.display = atBottom ? 'none' : 'flex';
}
...
function onTextareaKeydown(e) {
  ...
  if (e.key === 'Enter') {
    ...
    const insertStr = '\n' + indent + extraStr;
    textareaEl.value = text.substring(0, start) + insertStr + text.substring(end);
...
function runAC() {
  ...
  if (type === 'prop') {
    ...
    candidates = query ? values.filter(v => v.startsWith(query)).slice(0, 12) : values.slice(0, 12);
  }

  if (!candidates.length || (candidates.length === 1 && candidates[0] === query)) {
    hideAC(); return;
  }
...
function getACContext(text, pos) {
  ...
  if (colonIdx === -1) {
    ...
    return {
      ...
      prop: propRaw,
      query: valueRaw,
      insertStart: lineStart + colonIdx + 1 + (afterColon.length - valueRaw.length),
      insertEnd:   lineStart + lineText.length,
    };
  }
}
...
function showAC(items, query, insertStart, insertEnd, type) {
  ...
  items.forEach((item, i) => {
    ...
    el.innerHTML = matchLen
      ? '<span class="ci-ac-match">' + escHtml(item.substring(0, matchLen)) + '</span>' + escHtml(item.substring(matchLen))
      : escHtml(item);
    ...
    el.addEventListener('mousedown', (e) => { e.preventDefault(); acConfirmItem(el); });
...
function acConfirmItem(el) {
  ...
  textareaEl.value  = textareaEl.value.substring(0, insertStart) + finalValue + textareaEl.value.substring(insertEnd);
...
export function setCssProperty(selector, prop, value) {
  if (!textareaEl) return;
  textareaEl.value = updateCssString(textareaEl.value, selector, { [prop]: value + ' !important' });
  updateLineCount();
}
...
function updateLineCount() {
  ...
  const el = document.getElementById('ci-line-count');
  if (el) el.textContent = lines + ' line' + (lines !== 1 ? 's' : '');
}

function flash(msg, color = '#4caf82') {
  const el = document.getElementById('ci-flash-raw');
  if (!el) return;
  el.textContent = msg;
