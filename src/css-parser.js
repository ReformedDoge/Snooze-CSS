export function updateCssString(css, selector, propsObj) {
  ...
  for (let i = 0; i < css.length; i++) {
    ...
    if (inComment && char === '*' && nextChar === '/') {
      inComment = false; i++; selectorStart = i + 1; continue;
    }
    ...
    if (char === '{') {
      if (depth === 0) {
        ...
        if (currentSelector === selector || currentSelector.endsWith(selector)) {
          blockStart = i;
        }
      }
      depth++;
  ...
  if (blockStart !== -1 && blockEnd !== -1) {
    ...
    for (const [prop, val] of Object.entries(propsObj)) {
      ...
      if (propRegex.test(inner)) {
        ...
        if (inner.trim().length > 0 && !inner.trim().endsWith(';')) inner = inner.trimEnd() + ';';
        inner += `\n\t${prop}: ${val};`;
      }
    }
