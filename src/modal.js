...
import { buildSettingsTab, loadSettings, startMonitor, getSettings } from './settings.js';
...
export function createModal() {
  if (modalHost) {
    if (minimized) { restoreModal(); return; }
  ...
  modalHost.style.cssText = 'position: fixed; inset: 0; z-index: 999999; pointer-events: none;';
  ...
  const initW = 720, initH = Math.round(window.innerHeight * 0.82);
  ...
  modal.querySelectorAll('.ci-tab').forEach(btn => {
    btn.addEventListener('click', () => {
      ...
      if (btn.dataset.tab === 'builder' && prev !== 'builder') refreshValues('tab-switch');
    });
  });
  ...
  // ESC closes
  const onKey = (e) => { if (e.key === 'Escape') destroyModal(); };
  ...
  loadSettings().then(s => { if (s.autoMonitor) startMonitor(); });
}

function toggleMinimize() {
  if (!modalEl || !shadowRoot) return;
  
  // Notice we use shadowRoot.querySelector now!
...
export function destroyModal() {
  if (modalHost) {
    if (modalHost._onKey) document.removeEventListener('keydown', modalHost._onKey);
    modalHost.remove(); 
    modalHost  = null;
    shadowRoot = null;
    modalEl    = null;
